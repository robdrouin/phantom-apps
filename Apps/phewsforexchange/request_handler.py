import os, json, requests, encryption_helper
...
class Office365RequestHandler:
    ...
    def _get_oauth_token(self, code):
        ...
        body = {'grant_type': 'authorization_code', 
           ...
           'code': code}
        try:
            r = requests.post(request_url + '/token', data=body)
            r.raise_for_status()
            resp_json = r.json()
        except Exception as e:
            return self._return_error(('Error retrieving OAuth Token: {}').format(str(e)), 401)
...
class RequestStateHandler:
    ...
    def _encrypt_state(self, state):
        if 'oauth_token' in state:
            ...
            state['oauth_token'] = encryption_helper.encrypt(json.dumps(oauth_token), self._asset_id)
        return state

    def _decrypt_state(self, state):
        if 'oauth_token' in state:
            oauth_token = encryption_helper.decrypt(state['oauth_token'], self._asset_id)
