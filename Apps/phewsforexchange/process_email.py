...
import phantom.app, phantom.utils as ph_utils, mimetypes, socket
from email.header import decode_header
from phantom.vault import Vault
import shutil, hashlib, json, magic
from requests.structures import CaseInsensitiveDict
from copy import deepcopy
_container_common = {'run_automation': False}
_artifact_common = {'run_automation': False}
FILE_EXTENSIONS = {'.vmsn': [
           'os memory dump', 'vm snapshot file'], 
   '.vmss': [
           'os memory dump', 'vm suspend file'], 
   '.js': [
         'javascript'], 
   '.doc': [
          'doc'], 
...
class ProcessEmail(object):
    ...
    def _parse_email_headers_as_inline(self, file_data, parsed_mail, charset, email_id):
        p = re.compile('.*Forwarded Message.*\\r\\n(.*)', re.IGNORECASE)
        email_text = p.sub('\\1', file_data.strip()[:500])
    ...
    def _decode_uni_string(self, input_str, def_name):
        encoded_strings = re.findall('=\\?.*?\\?=', input_str, re.I)
    ...
    def _handle_if_body(self, content_disp, content_id, content_type, part, bodies, file_path):
        ...
        if content_disp is None:
            ...
            if content_disp.lower().strip() == 'inline':
    ...
    def _handle_part(self, part, part_index, tmp_dir, extract_attach, parsed_mail, child=False):
        ...
        file_path = ('{0}/{1}_{2}_{3}').format(tmp_dir, part_index, file_name.translate(None, ('').join(['<', '>', ' '])), child)
        ...
        if mail.is_multipart():
            ...
            for i, part in enumerate(mail.walk()):
                ...
                if message_id is None and part.get('Message-ID'):
                    ...
                    if message_id and part.get('Message-ID'):
                        child = True
    ...
    def _set_email_id_contains(self, email_id):
        ...
        if self._base_connector.get_app_id() == EXCHANGE_ONPREM_APP_ID and email_id.endswith('='):
            self._email_id_contains = [
             'exchange email id']
        else:
            if self._base_connector.get_app_id() == OFFICE365_APP_ID and email_id.endswith('='):
                self._email_id_contains = [
                 'office 365 email id']
            else:
                if self._base_connector.get_app_id() == IMAP_APP_ID and email_id.isdigit():
                    self._email_id_contains = [
                     'imap email id']
                else:
    ...
    def _save_ingested(self, container, using_dummy):
        if using_dummy:
            ...
            self._base_connector.debug_print(('save_container (with artifacts) returns, value: {0}, reason: {1}, id: {2}').format(ret_val, message, cid))
        return (
         ret_val, message, cid)
    ...
    def _add_vault_hashes_to_dictionary(self, cef_artifact, vault_id):
        ...
        try:
            ...
            return (
             phantom.APP_ERROR, 'Failed to get vault item metadata')
        else:
            try:
        ...
        return (
         phantom.APP_SUCCESS, 'Mapped hash values')
    ...
    def _handle_file(self, curr_file, vault_ids, container_id, artifact_id, run_automation=False):
        ...
        try:
            ...
            return (
             phantom.APP_ERROR, phantom.APP_ERROR)
        else:
            ...
            if phantom.APP_JSON_HASH in vault_ret:
                cef_artifact.update({'vaultId': vault_ret[phantom.APP_JSON_HASH], 'cs6': vault_ret[phantom.APP_JSON_HASH], 
                   'cs6Label': 'Vault ID'})
