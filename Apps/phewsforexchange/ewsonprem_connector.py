...
import ews_soap, requests, json, xmltodict, os, uuid
...
import email, urllib, imp, time
from request_handler import RequestStateHandler, _get_dir_name_from_app_name
app_dir = os.path.dirname(os.path.abspath(__file__))
os.sys.path.insert(0, ('{}/dependencies/ews_dep').format(app_dir))
...
class EWSOnPremConnector(BaseConnector):
    ...
    def _get_ping_fed_request_xml(self, config):
        try:
            ...
            return (
             None, ('Unable to create request xml data. Error: {0}').format(str(e)))
    ...
    def _set_federated_auth(self, config):
        ...
        try:
            ...
            return (
             None, ('Unable to send POST to ping url: {0}, Error: {1}').format(url, str(e)))
        else:
            ...
            data = {'resource': ('{0}://{1}').format(parsed_auth_url.scheme, parsed_auth_url.netloc), 
               'client_id': client_id, 
            ...
            try:
                ...
                return (
                 None, ('Failed to acquire token. POST request failed for {0}, Error: {1}').format(url, str(e)))
            else:
                if r.status_code != 200:
                ...
                try:
                    ...
                    return (
                     None, ('Unable to parse auth token response as JSON. Error: {0}').format(str(e)))
        ...
        return (
         OAuth2TokenAuth(resp_json['access_token'], resp_json['token_type']), '')
    ...
    def _make_rest_calls_to_phantom(self, action_result, url):
        ...
        try:
            ...
            return (
             action_result.set_status(phantom.APP_ERROR, 'Error processing response JSON', e), None)
    ...
    def _get_url_to_app_rest(self, action_result=None):
        ...
        return (
         phantom.APP_SUCCESS, url_to_app_rest)
    ...
    def _azure_int_auth_initial(self, client_id, rsh):
        ...
        return (
         OAuth2TokenAuth(oauth_token['access_token'], oauth_token['token_type']), '')
    ...
    def _azure_int_auth_refresh(self, client_id):
        ...
        try:
            ...
            return (
             None, ('Error refreshing token: {}').format(str(e)))
        else:
            try:
                oauth_token = r.json()
            except Exception as e:
    ...
    def _set_azure_int_auth(self, config):
        ...
        asset_id = self.get_asset_id()
        rsh = RequestStateHandler(asset_id)
    ...
    def _set_azure_auth(self, config):
        ...
        try:
            ...
            try:
                ...
                data = {'resource': ('{0}://{1}').format(parsed_base_url.scheme, parsed_base_url.netloc), 
                   'client_id': config[EWS_JSON_CLIENT_ID], 
    ...
    def initialize(self):
        ...
        if auth_type == AUTH_TYPE_AZURE:
            ...
            if auth_type == AUTH_TYPE_AZURE_INTERACTIVE:
                ...
                if auth_type == AUTH_TYPE_FEDERATED:
    ...
    def _clean_xml(self, input_xml):
        replace_regex = '&#x([0-8]|[b-cB-C]|[e-fE-F]|1[0-9]|1[a-fA-F]);'
    ...
    def _make_rest_call(self, result, data, check_response, data_string=False):
        ...
        try:
            ...
            return (
             result.set_status(phantom.APP_ERROR, EWSONPREM_ERR_SERVER_CONNECTION, e), resp_json)
        else:
            ...
            if not 200 <= r.status_code <= 399:
                ...
                return (
                 result.set_status(phantom.APP_ERROR, ('Call failed with HTTP Code: {0}. Reason: {1}. Details: {2}').format(r.status_code, r.reason, detail)), None)
            try:
                resp_json = xmltodict.parse(self._clean_xml(r.text))
                resp_json = json.loads(json.dumps(resp_json))
            except Exception as e:
                ...
                return (
                 result.set_status(phantom.APP_ERROR, msg_string, e), resp_json)
            else:
    ...
    def _get_child_folder_infos(self, user, action_result, parent_folder_info):
        ...
        for curr_step_value in xrange(0, 10000, step_size):
            ...
            if total_items == '0':
                return (
                 action_result.set_status(phantom.APP_ERROR, 'Children not found, possibly not present.'), None)
            ...
            if type(folders) != list:
                folders = [
                 folders]
            folder_infos.extend([ {'id': x['t:FolderId']['@Id'], 'display_name': x['t:DisplayName'], 'children_count': x['t:ChildFolderCount'], 'folder_path': self._extract_folder_path(x.get('t:ExtendedProperty'))} for x in folders
                                ])
        ...
        return (
         phantom.APP_SUCCESS, folder_infos)
    ...
    def _run_query_aqs(self, param):
        ...
        self.save_progress(('Searching in {0}\\{1}{2}').format(self._clean_str(user), folder_path if folder_path else 'All Folders', ' (and the children)' if not ignore_subfolders else ''))
        ...
        for i, folder_info in enumerate(folder_infos):
            ...
            if aqs:
                ...
                data = ews_soap.get_search_request_filter([folder_id], subject=subject, sender=sender, body=body, int_msg_id=int_msg_id, email_range=email_range)
    ...
    def _get_email_data_from_container(self, container_id, action_result):
        ...
        if not email_data:
            return RetVal3(action_result.set_status(phantom.APP_ERROR, 'Container does not seem to be created by the same app, raw_email data not found.'), None, None)
        if not email_id.endswith('=') and not ph_utils.is_sha1(email_id):
            return RetVal3(action_result.set_status(phantom.APP_ERROR, 'Container does not seem to be created by the same app, email id not in proper format.'), None, None)
    ...
    def _get_email(self, param):
        ...
        if recipients_mailbox and type(recipients_mailbox) != list:
            message['t_ToRecipients']['t_Mailbox'] = [
             recipients_mailbox]
        summary = {'subject': message.get('t_Subject'), 'create_time': message.get('t_DateTimeCreated'), 
    ...
    def _get_root_folder_id(self, user, action_result, is_public_folder=False):
        ...
        return (
         phantom.APP_SUCCESS, folder_info)
    ...
    def _get_folder_info(self, user, folder_path, action_result, is_public_folder=False):
        ...
        for i, folder_name in enumerate(folder_names):
            ...
            if not folder:
                return (
                 action_result.set_status(phantom.APP_ERROR, ("Information for folder '{0}' not found in response, possibly not present").format(curr_valid_folder_path)), None)
            folder_id = folder.get('t:FolderId', {}).get('@Id')
            if not folder_id:
                return (
                 action_result.set_status(phantom.APP_ERROR, ("Folder ID information not found in response for '{0}', possibly not present").format(curr_valid_folder_path)), None)
            parent_folder_id = folder_id
            folder_info = {'id': folder_id, 'display_name': folder.get('t:DisplayName'), 
    ...
    def _extract_ext_properties_from_attachments(self, resp_json):
        ...
        return (
         phantom.APP_SUCCESS, email_headers_ret, attach_meta_info_ret)
    ...
    def _parse_email(self, resp_json, email_id, target_container_id):
        try:
            ...
            return (
             phantom.APP_ERROR, 'Email MimeContent missing in response.')
        else:
            try:
                rfc822_email = base64.b64decode(mime_content)
            except Exception as e:
                self.debug_print('Unable to decode Email Mime Content', e)
                return (
                 phantom.APP_ERROR, 'Unable to decode Email Mime Content')
        ...
        process_email = ProcessEmail()
        return process_email.process_email(self, rfc822_email, email_id, self.get_config(), epoch, target_container_id, email_headers=email_header_list, attachments_data=attach_meta_info_list)
    ...
    def _get_email_infos_to_process(self, offset, max_emails, action_result, restriction=None):
        ...
        if items is None:
            self.debug_print('items is None')
            return (
             action_result.set_status(phantom.APP_SUCCESS, 'Result does not contain items key. Possibly no emails in folder'), None)
        items = resp_json.get('t:Items', {}).get('t:Message', [])
        if type(items) != list:
            items = [
             items]
        ...
        return (
         phantom.APP_SUCCESS, email_infos)
    ...
    def _on_poll(self, param):
        ...
        if email_ids and config[EWS_JSON_INGEST_MANNER] == EWS_INGEST_OLDEST_EMAILS:
            ...
            if len(email_times) == 1:
                self.debug_print('Getting all emails with the same LastModifiedTime, down to the second.' + (' That means the device is generating max_containers=({0}) per second.').format(max_emails) + ' Skipping to the next second to not get stuck.')
    ...
    def handle_action(self, param):
        ...
        if action == self.ACTION_ID_RUN_QUERY:
            ...
            if action == self.ACTION_ID_DELETE_EMAIL:
                ret_val = self._delete_email(param)
            else:
                if action == self.ACTION_ID_UPDATE_EMAIL:
                    ret_val = self._update_email(param)
                else:
                    if action == self.ACTION_ID_GET_EMAIL:
                        ret_val = self._get_email(param)
                    else:
                        if action == self.ACTION_ID_COPY_EMAIL:
                            ret_val = self._copy_move_email(param)
                        else:
                            if action == self.ACTION_ID_MOVE_EMAIL:
                                ret_val = self._copy_move_email(param, action='move')
                            else:
                                if action == self.ACTION_ID_EXPAND_DL:
                                    ret_val = self._expand_dl(param)
                                else:
                                    if action == self.ACTION_ID_RESOLVE_NAME:
                                        ret_val = self._resolve_name(param)
                                    else:
                                        if action == self.ACTION_ID_ON_POLL:
...
if __name__ == '__main__':
    ...
    with open(args.input_test_json) as (f):
        ...
        if raw_email:
            config = {'extract_attachments': True, 'extract_domains': True, 
               'extract_hashes': True, 
               'extract_ips': True, 
               'extract_urls': True, 
