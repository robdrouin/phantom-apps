...
def xml_get_restriction(greater_than_time=None, message_id=None):
    ...
    if greater_than_time:
        greater_than_time = T.IsGreaterThan(T.FieldURI({'FieldURI': 'item:LastModifiedTime'}), T.FieldURIOrConstant(T.Constant({'Value': greater_than_time})))
...
def xml_get_email_ids(user, folder_id, order, offset, max_emails, restriction):
    ...
    parent_folder_ids = M.ParentFolderIds(T.DistinguishedFolderId({'Id': folder_id}, T.Mailbox(T.EmailAddress(user.decode('utf-8')))))
    if folder_id != 'inbox':
...
def xml_get_attachments_data(attachment_ids_to_query):
    ...
    """
    additional_properties = T.AdditionalProperties(T.FieldURI({'FieldURI': 'message:Sender'}), T.FieldURI({'FieldURI': 'message:InternetMessageId'}), T.FieldURI({'FieldURI': 'item:DateTimeReceived'}), T.FieldURI({'FieldURI': 'item:Attachments'}), T.ExtendedFieldURI({'PropertyTag': EXTENDED_PROPERTY_HEADERS, 'PropertyType': 'String'}), T.ExtendedFieldURI({'PropertyTag': EXTENDED_PROPERTY_BODY_TEXT, 'PropertyType': 'String'}), T.FieldURI({'FieldURI': 'item:LastModifiedTime'}))
...
def xml_get_emails_data(email_ids):
    ...
    PropertyTag 0x007D is required, which points to PR_TRANSPORT_MESSAGE_HEADERS
    """
    additional_properties = T.AdditionalProperties(T.FieldURI({'FieldURI': 'item:Subject'}), T.FieldURI({'FieldURI': 'message:From'}), T.FieldURI({'FieldURI': 'message:Sender'}), T.FieldURI({'FieldURI': 'message:InternetMessageId'}), T.FieldURI({'FieldURI': 'item:Categories'}), T.ExtendedFieldURI({'PropertyTag': EXTENDED_PROPERTY_HEADERS, 'PropertyType': 'String'}), T.ExtendedFieldURI({'PropertyTag': EXTENDED_PROPERTY_BODY_TEXT, 'PropertyType': 'String'}), T.FieldURI({'FieldURI': 'item:DateTimeReceived'}), T.FieldURI({'FieldURI': 'item:LastModifiedTime'}), T.FieldURI({'FieldURI': 'item:Body'}))
...
def get_search_request_aqs(folder_ids, aqs, email_range='0-10'):
    elements = []
    additional_properties = T.AdditionalProperties(T.FieldURI({'FieldURI': 'item:Subject'}), T.FieldURI({'FieldURI': 'message:From'}), T.FieldURI({'FieldURI': 'message:Sender'}), T.FieldURI({'FieldURI': 'message:InternetMessageId'}), T.FieldURI({'FieldURI': 'item:DateTimeReceived'}), T.ExtendedFieldURI({'PropertySetId': 'aa3df801-4fc7-401f-bbc1-7c93d6498c2e', 'PropertyName': 'ItemIndex', 'PropertyType': 'Integer'}))
    item_shape = M.ItemShape(T.BaseShape('IdOnly'), additional_properties)
    elements.append(item_shape)
...
def get_search_request_filter(folder_ids, subject=None, sender=None, body=None, int_msg_id=None, restriction=None, email_range='0-10'):
    ...
    if restriction is None:
        filters = []
        if subject:
            sub_filt = T.Contains({'ContainmentMode': 'Substring', 'ContainmentComparison': 'IgnoreCase'}, T.FieldURI({'FieldURI': 'item:Subject'}), T.Constant({'Value': subject.decode('utf-8')}))
            filters.append(sub_filt)
        if sender:
            sender_filter = T.IsEqualTo(T.FieldURI({'FieldURI': 'message:Sender'}), T.FieldURIOrConstant(T.Constant({'Value': sender.decode('utf-8')})))
            filters.append(sender_filter)
        if int_msg_id:
            sender_filter = T.IsEqualTo(T.FieldURI({'FieldURI': 'message:InternetMessageId'}), T.FieldURIOrConstant(T.Constant({'Value': int_msg_id.decode('utf-8')})))
            filters.append(sender_filter)
        if body:
            body_filter = T.Contains({'ContainmentMode': 'Substring', 'ContainmentComparison': 'IgnoreCase'}, T.FieldURI({'FieldURI': 'item:Body'}), T.Constant({'Value': body.decode('utf-8')}))
...
def get_update_email(email_id, change_key, categories=None, subject=None):
    ...
    update_item = M.UpdateItem({'MessageDisposition': 'SaveOnly', 'ConflictResolution': 'AlwaysOverwrite'}, M.ItemChanges(T.ItemChange(item_id, T.Updates(*update_node))))
    return update_item
...
def xml_get_root_folder_id(user, root_folder_id='root'):
    ...
    if root_folder_id == 'publicfoldersroot':
        ...
        par_folder_id = M.ParentFolderIds(T.DistinguishedFolderId({'Id': root_folder_id}, T.Mailbox(T.EmailAddress(user.decode('utf-8')))))
...
def xml_get_children_info(user, child_folder_name=None, parent_folder_id='root', query_range=None):
    elements = []
    folder_shape = M.FolderShape(T.BaseShape('IdOnly'), T.AdditionalProperties(T.FieldURI({'FieldURI': 'folder:FolderId'}), T.FieldURI({'FieldURI': 'folder:FolderClass'}), T.FieldURI({'FieldURI': 'folder:ChildFolderCount'}), T.FieldURI({'FieldURI': 'folder:ParentFolderId'}), T.ExtendedFieldURI({'PropertyTag': '26293', 'PropertyType': 'String'}), T.FieldURI({'FieldURI': 'folder:DisplayName'})))
    ...
    if child_folder_name:
        display_name_equal_to = T.IsEqualTo(T.FieldURI({'FieldURI': 'folder:DisplayName'}), T.FieldURIOrConstant(T.Constant({'Value': child_folder_name.decode('utf-8')})))
    ...
    if user:
        if parent_folder_id == 'root':
            par_folder_id = M.ParentFolderIds(T.DistinguishedFolderId({'Id': parent_folder_id}, T.Mailbox(T.EmailAddress(user.decode('utf-8')))))
...
def add_to_envelope(lxml_obj, target_user=None):
    ...
    if target_user:
        impersonation = T.ExchangeImpersonation(T.ConnectingSID(T.SmtpAddress(target_user.decode('utf-8'))))
